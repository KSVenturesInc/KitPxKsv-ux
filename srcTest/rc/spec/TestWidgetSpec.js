...
define(['baja!',
  ...
  describe('nmodule/KitPxKsv/rc/TestWidget', function () {
    var stooges,
        widget,
        elem;
    ...
    describe('#doInitialize()', function () {
      it('creates the structure to load buttons into', function () {
        expect(elem.find('.TestWidget-header').length).toBe(1);
        expect(elem.find('.TestWidget-content').length).toBe(1);
      });

      it('arms a handler to set active class on button click', function () {
        var contentDom = elem.find('.TestWidget-content'),
      ...
      it('arms a handler to display selected slot name', function () {
        var slotDom = elem.find('.TestWidget-selected-slot');

        runs(function () {
          var contentDom = elem.find('.TestWidget-content'),
              button = $('<button class="TestWidget-button" data-slot="curlyJoe"></button>');
